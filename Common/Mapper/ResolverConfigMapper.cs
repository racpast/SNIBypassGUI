...
{
    public class ResolverConfigMapper(IMapper<HttpHeaderItem> httpHeaderItemMapper) : IMapper<ResolverConfig>
    {
        /// <summary>
        /// 将 <see cref="ResolverConfig"/> 类型的 <paramref name="resolverConfig"/> 实例转换为 JSON 对象。
        /// </summary>
        public JObject ToJObject(ResolverConfig resolverConfig)
        {
            var jObject = new JObject
            {
                ["id"] = resolverConfig.Id.ToString(),
                ["configName"] = resolverConfig.ConfigName.OrDefault(),
            ...
            switch (resolverConfig.ProtocolType)
            {
                ...
                case ResolverConfigProtocol.DnsOverHttps:
                    ...
                    jObject["httpHeaders"] = new JArray(resolverConfig.HttpHeaders?.Select(httpHeaderItemMapper.ToJObject).OrEmpty());
        ...
        /// </summary>
        public ParseResult<ResolverConfig> FromJObject(JObject jObject)
        {
            if (jObject == null)
            ...
            switch (protocolType)
            {
                ...
                case ResolverConfigProtocol.DnsOverHttps:
                    ...
                    {
                        var parsed = httpHeaderItemMapper.FromJObject(item);
                        if (!parsed.IsSuccess)
